...
static inline void lcd_send_cmd(uint8_t);
static inline void lcd_send_data(uint8_t *, uint16_t);
static inline void lcd_send_color_565(uint16_t, uint32_t);
...
void lcd_print(uint8_t *, uint16_t, uint8_t);
...
static inline uint16_t touch_send_data(uint8_t);
