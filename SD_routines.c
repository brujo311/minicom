...
unsigned char SD_init(void)
{
  ...
  do
  {
    ...
    if(retry>0xfe)
    {
      ...
      if(SD_status & (1 << SD_ON) && SD_status & (1 << SD_INSERTED)) { console_write("SD card removed\n\0"); SD_status &= ~((1 << SD_INSERTED) | (1 << FAT32_ACTIVE)); }
      else if(!(SD_status & (1 << SD_ON))) { console_write("SD card not detected...\n\n\0"); }
        return 1;
    } //time out
  ...
  if(SD_status & (1 << SD_ON) && !(SD_status & (1 << SD_INSERTED))) { console_write("SD card inserted\n\0"); SD_status |= (1 << SD_INSERTED); }
  else if(!(SD_status & (1 << SD_ON))) { console_write("SD card detected\n\0"); SD_status |= (1 << SD_ON) | (1 << SD_INSERTED); }
...
SPI_send(cmd | 0x40); //send command, first two bits always '01'
SPI_send(arg>>24);
SPI_send(arg>>16);
SPI_send(arg>>8);
SPI_send(arg);
...
while((response = SPI_send(0xff)) == 0xff) //wait response
...
retry = 0;
while(SPI_send(0xff) != 0xfe) //wait for start block token 0xfe (0x11111110)
  if(retry++ > 0xfffe){SD_CS_DEASSERT; return 1;} //return if time-out

for(i=0; i<512; i++) //read 512 bytes
  buffer[i] = SPI_send(0xff);

SPI_send(0xff); //receive incoming CRC (16-bit), CRC is ignored here
SPI_send(0xff);

SPI_send(0xff); //extra 8 clock pulses
SD_CS_DEASSERT;
...
SPI_send(0xfe);     //Send start block token 0xfe (0x11111110)

for(i=0; i<512; i++)    //send 512 bytes data
  SPI_send(buffer[i]);

SPI_send(0xff);     //transmit dummy CRC (16-bit), CRC is ignored here
SPI_send(0xff);
...
while(!SPI_send(0xff)) //wait for SD card to complete writing and get idle
if(retry++ > 0xfffe){SD_CS_DEASSERT; return 1;}

SD_CS_DEASSERT;
SPI_send(0xff);   //just spend 8 clock cycle delay before reasserting the CS line
...
while( blockCounter < totalBlocks )
{
   ...
   SPI_send(0xfc); //Send start block token 0xfc (0x11111100)
   ...
   SPI_send(0xff); //extra 8 bits
   blockCounter++;
}

SPI_send(0xfd); //send 'stop transmission token'
...
SD_CS_DEASSERT;
SPI_send(0xff); //just spend 8 clock cycle delay before reasserting the CS signal
